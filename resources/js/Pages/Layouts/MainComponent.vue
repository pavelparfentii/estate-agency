<script setup>
    import {Link, usePage} from "@inertiajs/vue3";
    import {computed, ref} from "vue";
    // page.props.value.flash.success
    const page = usePage();

    const user = computed(() => page.props.user);

    const notificationCount = computed(()=> Math.min(page.props.user.notificationCount, 9))

    const success = computed(() => {
        return page.props.flash.success;
    });

    const timer = ref(0);
    setInterval(()=> timer.value++, 1000);
</script>

<template>



    <header class="border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 w-full">
        <div class="container m-auto w-full">
            <nav class="p-4 flex items-center justify-between">
                <div class="text-lg font-medium">
<!--                    <Link href="/listing">Main Page</Link>&nbsp;-->
                    <Link :href="route('listing.index')">Listing</Link>&nbsp;
                </div>
                <div></div>
                <div class="text-xl text-indigo-600 dark:text-indigo-400 font-bold text-center">
                    <!--                    <Link href="/listing">Main Page</Link>&nbsp;-->
                    <Link :href="route('listing.index')">EstateAgency</Link>&nbsp;
                </div>
                <div v-if="user" class="flex items-center gap-4">
                    <Link  class="text-gray-500 relative pr-2 py-2 text-lg" :href="route('notification.index')">
                        🔔
                        <div v-if="notificationCount" class="absolute right-0 top-0 w-5 h-5 bg-red-700 dark:bg-red-400 text-white font-medium border-white dark:border-gray-900 rounded-full text-xs text-center">
                            {{notificationCount}}
                        </div>
                    </Link>
<!--                    <Link href="/listing/create">Create</Link>-->
                    <Link  class="text-gray-500" :href="route('realtor.listing.index')">{{user.name}}</Link>
                    <Link :href="route('realtor.listing.create')" class="btn-primary">+ New Listing</Link>
                    <div>
                        <Link :href="route('logout')" method="delete" as="button">Logout</Link>
                    </div>
                </div>
                <div v-else class="flex items-center gap-4">
                    <Link :href="route('user-account.create')" > Register</Link>
                    <Link :href="route('login')" > Sign In</Link>
                </div>
            </nav>
        </div>
    </header>
<!--    <div>The timer value is {{timer}}</div>-->
    <main class="container mx-auto p-4">
        <div v-if="success" class="mb-4 border rounded-b-md shadow-sm border-green-200 dark:border-green-700 bg-green-50 dark:bg-green-600 p-2">
            {{success}}
        </div>
        <slot>Default</slot>
    </main>

</template>

<style scoped>
    .success{
        background-color: green;
        color: white;
    }
</style>

